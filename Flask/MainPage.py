...
def setClientData(email,nom,prénom,sex,balance,incomes,expenses):
	new_client=Client(Email=email,Name=nom,LastName=prénom,Sex=sex,CurrentBalance=balance,Incomes=balance,Expenses=expenses)
	if new_client:
		...
		for x in getData :
			if str(x.Email)=='tst@aa.aa' :
				deleted=Client.query.filter_by(Email='tst@aa.aa').delete()
				if deleted:
